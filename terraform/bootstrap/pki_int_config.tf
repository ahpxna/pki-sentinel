...
# CRL/OCSP behaviour. delta_rebuild_interval=1m shortens the window between
# revocation and CRL visibility — the Phase 3 probe measures exactly this.
# ocsp_expiry=1h trades responder load for faster propagation; both are
# discussed in docs/benchmarks/ocsp-softfail-thresholds.md.
...
resource "vault_generic_endpoint" "int_cluster_config" {
  ...
  data_json = jsonencode({
    path     = "${var.vault_public_addr}/v1/${vault_mount.pki_int.path}"
